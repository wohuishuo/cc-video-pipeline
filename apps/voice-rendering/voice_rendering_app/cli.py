...
from .adapters import EdgeTtsAdapter
...
def parser():
    value = argparse.ArgumentParser(description="Render one Edge TTS clip per translated segment serially.")
...
def main(argv=None, *, adapter_factory=None):
    ...
    adapter = (adapter_factory or (lambda options: EdgeTtsAdapter(rate=options.rate, volume=options.volume)))(args)
