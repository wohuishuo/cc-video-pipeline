...
def parser():
    ...
    plan=commands.add_parser("plan"); plan.add_argument("video",type=Path); plan.add_argument("--metadata",type=Path,required=True); plan.add_argument("--target",action="append",required=True,help="platform=account"); plan.add_argument("--credential",action="append",default=[],help="platform=credential-id reference"); plan.add_argument("--public",action="store_true"); plan.add_argument("--output-dir",type=Path,required=True); plan.add_argument("--operation-id",required=True); plan.add_argument("--json",action="store_true")
    execute=commands.add_parser("execute"); execute.add_argument("plan",type=Path); execute.add_argument("--confirmation",required=True); execute.add_argument("--credential-vault",type=Path); execute.add_argument("--platform-io-launcher",type=Path); execute.add_argument("--output-dir",type=Path,required=True); execute.add_argument("--operation-id",required=True); execute.add_argument("--json",action="store_true")
    return root
...
def credentials(values):
    result={}
    for value in values:
        platform,separator,credential_id=value.partition("=")
        if not separator or platform not in PLATFORMS or platform in result: raise PublicationError("each --credential must be a unique platform=credential-id")
        result[platform]=credential_id
    return result
...
def main(argv=None):
    ...
    try:
        if args.command=="plan":
            result=PublicationPlanner().execute(PlanSpec.create(args.video,args.metadata,targets(args.target),credentials=credentials(args.credential),public=args.public),args.output_dir,args.operation_id); artifact=result.plan_path
        else:
