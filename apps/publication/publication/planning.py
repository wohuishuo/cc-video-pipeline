...
class PublicationPlanner:
    def execute(self,spec:PlanSpec,output_dir,operation_id):
        ...
        for ordinal,(platform,account) in enumerate(spec.targets,1):
            identity=hashlib.sha256(f"{video_sha}\0{metadata_sha}\0{platform}\0{account}\0{visibility}".encode()).hexdigest()
            jobs.append({"ordinal":ordinal,"id":identity,"platform":platform,"account":account,"visibility":visibility})
