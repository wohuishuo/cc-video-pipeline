...
| 视频 skills | `_refs/jianshuo-claude-skills/` | 王建硕全套：剪辑/字幕/配音/横转竖/切片/封面/发布… |
| Awesome 索引 | `_refs/awesome-claude-skills/README.md` | 全网 skill 精选目录（带链接，按分类） |
| Awesome 索引2 | `_refs/awesome-agent-skills/` | 1000+ agent skill 目录（跨 Claude/Codex/Cursor） |
...
## 安装方法（告诉用户三选一）
- **官方/marketplace**：在交互式 `claude` 终端跑 `/plugin marketplace add anthropics/skills`，再 `/plugin install <名>`
- **本地目录**：把某个 skill 文件夹复制进本项目 `.claude/skills/<名>/`（含 SKILL.md），重启 CCD 生效
- **从 _refs 采用**：`_refs` 里的是参考；要正式用就复制进 `.claude/skills/`（像我们移植 reframe 那样，注意 mac/bash → Windows/PowerShell 适配）

## 注意
- `_refs/` 是只读参考区，**不直接当生效 skill**；生效的在 `.claude/skills/`。
