...
### 1. 下载 + 抽音频
```powershell
.\.Codex\skills\ref-analyze\scripts\fetch.ps1 -Url "<链接>" -Slug "<up主-选题名>"
```
...
### 3. 客观信号 + 抽帧
```powershell
.\.Codex\skills\ref-analyze\scripts\probe.ps1 -Dir ".\reference\<slug>"
.\.Codex\skills\ref-analyze\scripts\extract_frames.ps1 -Video ".\reference\<slug>\video.mp4" -OutDir ".\reference\<slug>\frames" -Cuts ".\reference\<slug>\cuts.txt" -MaxFrames 120
```
产出：`cuts.txt`（镜头切换）、`rms.txt`（响度包络）、`frames/`（关键帧图片，供 Codex 视觉分析）。

### 4. 生成分析报告（这一步由我 Codex 完成）
