---
name: localize-video
description: 视频本地化编排器（P2，部分实现）。把一条视频本地化为目标语言：转录→翻译→（配音）→烧字幕→竖屏导出。当用户说"帮我把这个视频做成中文字幕/翻译成英文/配俄语音/全套本地化"时使用。注意：翻译/配音步骤尚未自动化，由 Codex 手工完成或待补脚本。
---

# 视频本地化 (localize-video) — 编排器（⚠️ 部分实现）

> 这是个**编排器**：本身不做活，只把任务路由到各步骤。
> 设计参考 `_refs/jianshuo-Codex-skills/wjs-localizing-video`（mac/bash 版，可照着补脚本）。
...
| 2. 翻译 | **由我 Codex 直接翻译 SRT** | ⚠️ 无脚本，手工。规则见下 |
...
.\tools\.venv\Scripts\python.exe .\.Codex\skills\localize-video\scripts\tts_dub.py `
...
# 4+5. 烧目标语言字幕 + 竖屏
.\.Codex\skills\edit-ffmpeg\scripts\to_vertical.ps1 -Video "$d\video.mp4" -Out "$d\out_ja_9x16.mp4" -Mode captions -Subtitle "$d\audio.ja.srt"
```
...
| 方向 | 转录引擎 | 翻译 | 配音(待补) | 字幕 |
|---|---|---|---|---|
| 中→俄/英/日 | FunASR zh | Codex | Edge TTS | edit-ffmpeg |
| 俄/英→中 | faster-whisper | Codex | Edge TTS zh | edit-ffmpeg |

## TODO（按需做，别提前堆）
- [x] `scripts/tts_dub.py`：Edge TTS 配音（免费、本地、中俄英日）✅
- [ ] `scripts/translate_srt.py`：若想脱离对话批量翻译，再接 Codex API
