...
| Evidence present | graph/run/process contracts; SQLite idempotency; durable FIFO start queue; queued cancellation isolation; real child-process ordering and cleanup; process-wide execution gate; optional Resource Budget composition; strict truncated-catalog rejection plus explicit partial-catalog admission; restored authentication-file references; exact local folder inventory including OneDrive roots; separately selectable NLLB/DeepSeek and Edge/Qwen3/original policies; zero-route local-output admission; seven-stage responsive browser drill; stage-specific operator actions and editable workload review; real loopback HTTP and Client Contracts discovery; secure fixed and multi-workspace admission; browser-admitted folder intake; successful anonymous YouTube download; real transcription, RU+KK translation and Source-to-Localization runs; real 75-item Douyin creator discovery; domain-tested Creator+Dub continuation and derivative verification; publication planning; confirmed private YouTube execution through Publication and Vault with a fake platform boundary; live process-tree crash, startup fence and same-run recovery |
| Evidence missing | live multi-item Creator+Dub browser run; live started multi-derivative Release run; power-loss/filesystem durability; representative resource/load limits; authenticated publication adapters; hosted identity and production tenant isolation; cross-workspace scheduling contract; attack-oriented security operations |
...
Browser run `5bfa7769-5f94-44b1-b6f1-26fce880bd4e` completed both Creator Discovery owner steps on 2026-08-03 after Studio restored that authentication-file reference. The request used `maxItems=0`, the platform adapter enumerated and verified 75 canonical Douyin videos, and the committed manifest SHA-256 was `593c0bc777dc991fe7d690c24a8d65680c009ac117798f62a76cca8402d9ce57`. The browser rendered `75 shown / 75 total` and logged no console errors.

The same browser drill first admitted the previously discovered three-item manifest only after explicit `allowPartialCatalog: true` consent. It projected three sources into six Russian/English Edge-TTS local outputs with zero publication routes. The API still rejected the same incomplete manifest when consent was omitted.

The version-13 interaction drill then selected the full 75-item catalog, retained Russian and English with Edge TTS, projected exactly 150 local MP4 outputs and zero publication routes, and enabled **Start local processing**. Review edit actions returned to their owning stages, desktop document width remained contained at 1932 pixels and the browser console remained empty. No media run was started.
...
## Creator Batch graph composition evidence

Browser/API run `e806b8b2-fa0c-4012-9495-03949a8340f6` was admitted as a four-owner `creator-batch-dub` Graph for `ru-RU,en-US`; the browser exposed the complete configuration, rendered every owner node and kept the start control visible without overflow. The run was not started, so this is composition evidence rather than live multi-item delivery evidence. See [the complete drill](creator-batch-graph-drill.md).

## Publication Batch graph composition evidence

The Folder/URL Release Graph composes the existing ten localization nodes with Publication Batch plan and independent verification nodes. Exact browser/API run evidence is recorded in [the complete drill](publication-batch-graph-drill.md). The admitted run was not started, so no download, synthesis, upload or platform contact is claimed.

## Publication Batch Execution graph composition evidence

Browser/API run `b90cd6af-9284-49bd-a3b8-c9bc07718a40` was admitted from completed Release run `5a9643d9-60ce-4e28-84f4-6ce03f512269` with the exact batch-plan SHA. At 1280 x 720 the nineteenth workflow remained contained inside a horizontally scrollable selector and the document width stayed equal to the viewport. The execution run was not started, so no credential injection or platform contact is claimed. See [the complete drill](publication-batch-execution-graph-drill.md).
