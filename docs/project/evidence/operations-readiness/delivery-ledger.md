...
| Evidence present | typed run/step state; optimistic versions; idempotent startup fence; deterministic restart tests; real server process-tree loss; same-SQLite restart; same-run resume; ordered logs and artifact SHA; clean listener shutdown; public immutable client-contract discovery before admission; digest-only workspace credentials; real secure loopback scope separation and wrong-workspace denial; real multi-workspace routing into disjoint SQLite/artifact roots; process-wide maximum execution concurrency one; durable local byte/slot reservations; real Studio reserve-before-child and terminal release composition |
| Evidence missing | abrupt machine power loss; filesystem corruption recovery; backup/restore; remote database failover; production tenant isolation; distributed quota reservations; credential-vault recovery; representative CPU/GPU/network load budgets; authenticated upload reconciliation; alerting and service-level objectives |
