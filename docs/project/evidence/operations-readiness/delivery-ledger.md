...
| Evidence present | typed run/step state; optimistic versions; idempotent startup fence; deterministic restart tests; real server process-tree loss; same-SQLite restart; same-run resume; ordered logs and artifact SHA; clean listener shutdown; digest-only workspace credentials; real secure loopback scope separation and wrong-workspace denial; independently verified disjoint workspace storage namespaces and capacity projection |
| Evidence missing | abrupt machine power loss; filesystem corruption recovery; backup/restore; remote database failover; Studio storage routing and production tenant isolation; hard quota reservations; credential-vault recovery; resource/load budgets; authenticated upload reconciliation; alerting and service-level objectives |
